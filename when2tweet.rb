...
#
# == Description
# * Migrated the script to RubyScripts API 1.1 from a shell script I found somewhere I just can't find again.
# * Script outputs the 4 top timeframes to reach your top 25 followers during their most active timeframe.
#
# === Usage:
#
...
# - access_token, access_secret: credentials for using authenticated twitter API methods
...
users_timeline_url = '/1.1/statuses/user_timeline.json?include_entities=true&include_rts=true&screen_name='


@consumer=OAuth::Consumer.new( consumer_key ,consumer_secret, {:site=>twitter_api_url})
...
vip_followers = users_data.sort_by{|keys,value| value}.reverse[0..25]  # top 25 followers
...
tweets_ts_wd.each do |tweets_ts|
  puts '%2.2f%% Activity at %s' %  [tweets_ts[1]*100,tweets_ts[0]]
end
puts '------------------------------------'
